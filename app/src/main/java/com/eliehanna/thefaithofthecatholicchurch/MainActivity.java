package com.eliehanna.thefaithofthecatholicchurch;
...
public class MainActivity extends AppCompatActivity {
    ...
    private class CustomWebViewClient extends WebViewClient {
        ...
        public boolean shouldOverrideUrlLoading(WebView view, String url) {
            ...
            // Check if the URL is external (not a local file or asset)
            if (url.startsWith("http://") || url.startsWith("https://")) {
                // Open external URLs (like GitHub) in the default browser
                Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(url));
                view.getContext().startActivity(intent);
                return true; // Indicate that we handled this URL externally
            }

            // For internal URLs, continue loading in the WebView
            view.loadUrl(url);
            return true;
        }
