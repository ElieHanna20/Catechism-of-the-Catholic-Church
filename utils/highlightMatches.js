const highlightMatches = (searchTerm, mainHtml, state, spanElements, mainElement, upButton, downButton, InputCount, removeHighlights, updateDisplay) =>
{
    // console.log('addHi');
    if (document.querySelector('mark')) removeHighlights(mainHtml, state.currentIndex, mainElement, upButton, downButton, InputCount);
    ...
    const escapedTerm = trimmedSearchTerm.replace(/([.*+?^${}()|[\]\\])/g, '\\$1');
    ...
    updateDisplay(state.currentIndex, spanElements, InputCount);
